...
export class TodosCuidadores implements OnInit {
  ...
  async carregarCuidadores(): Promise<void> {
    ...
    try {
      ...
      const res = await fetch('http://https://educainclusiva-backend-hvwz.onrender.com/usuarios/cuidadores', {
  ...
  async apagarCuidador(): Promise<void> {
    ...
    this.apagando = true;
    try {
      const res = await fetch(`http://https://educainclusiva-backend-hvwz.onrender.com/usuarios/${this.cuidadorParaApagar.id}`, {
        method: 'DELETE',
