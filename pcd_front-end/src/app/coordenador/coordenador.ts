...
export class Coordenador implements OnInit {
  ...
  carregarBoletimAluno(): void {
    ...
    fetch(`http://https://educainclusiva-backend-hvwz.onrender.com/boletim/${this.alunoSelecionado.id}?bimestre=${this.bimestreSelecionado}`, {
  ...
  async carregarAlunos(): Promise<void> {
    try {
      const token = localStorage.getItem('token');
      const res = await fetch('http://https://educainclusiva-backend-hvwz.onrender.com/alunos/todos', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
  ...
  async carregarTurmas(): Promise<void> {
    try {
      const token = localStorage.getItem('token');
      const res = await fetch('http://https://educainclusiva-backend-hvwz.onrender.com/escola/turmas', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const dados = await res.json();
      this.ngZone.run(() => {
  ...
  async carregarStats(): Promise<void> {
    try {
      const token = localStorage.getItem('token');
      const res = await fetch('http://https://educainclusiva-backend-hvwz.onrender.com/escola/stats', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const dados = await res.json();
      this.ngZone.run(() => {
  ...
  async carregarMensagens(): Promise<void> {
    try {
      const token = localStorage.getItem('token');
      const res = await fetch(`http://https://educainclusiva-backend-hvwz.onrender.com/recados/coordenador/${this.usuario.id}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const dados = await res.json();
      this.ngZone.run(() => {
